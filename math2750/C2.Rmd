---
title:  "Computational Practical 2 & Assessment 2"
author: "[Matthew Aldridge](mailto:m.aldridge@leeds.ac.uk)"
date:   "MATH2750 Introduction to Markov Processes (2019--20)"
output: html_document
---
<!-- or "output: pdf_document" to make a LaTeXed PDF -->
  
  
*This computer practical is an oppotunity to learn more about Markov chains using R. This practical doubles as **Assessment 2**, and is an assessed part of the course.*

*This assessment counts as 4% of your final module grade. You should write a report on your work (see Question 0 below), answering the questions and showing any relevant R code. The deadline for submission is **Thursday 26 March at 2pm**. You can write your report in any format you like, but your report should be handed in **on paper** to your marker's pigeonhole on level 8 of the School of Mathematics. (You can find out who your marker is in the red folder by the pigeonholes.) Please staple multiple pages. Your script must clearly show your name, your marker's name, and your student number. Scripts which cannot be identified may not be marked. Late submissions up to Friday 27 March at 2pm will still be marked, but the total mark will be reduced by 50%. You must email your marker to let them know you have submitted a late script. Scripts handed in after Friday 27 March at 2pm will not be marked.*

*Your report will by marked out of 4, with 1 mark each available for Questions 1--4. For each question, a mostly correct (or better) answer will receive 1 mark; a mostly incorrect (or worse) answer will receive 0 marks.*

*You should work though this problem sheet in the practical session for this course, between Monday 9 and Thursday 12 March. Check your timetable (look for week 20) to find the time and room for your session. (The webpage <https://leeds.service-now.com/it?id=clusters> will give you directions.) A member of staff will be present to help you with any difficulties. I recommend that one hour in the computer practical to work through most of the sheet plus one or two hours of your own time to finish the sheet and write up your report should be sufficient time to score full marks on this practical.*

*This question sheet is available in two formats*

* *[`C2.html`](https://mpaldridge.github.io/math2750/C2.html) is an easy-to-read HTML file of the questions and comments. You should open this in a web browser.*
* *[`C2.Rmd`](https://mpaldridge.github.io/math2750/C2.Rmd) is a plain text file. I recommend opening this file in RStudio. This will make it easy to run the R code included here, by clicking the green "play" button by each chunk of R code. (This file is written in a language called "R Markdown"; see the bottom of the document for more information on R Markdown.)*

*This question sheet assumes you can open RStudio (or operate R in whatever way you prefer), and that you can use basic commands such as `x <- c(1, 2, 3)` and `mean(x)`. The University of Leeds* [Introduction to R](http://www1.maths.leeds.ac.uk/statistics/R/Rintro.pdf) *covers more basic topics.*



***



**Question 0:** *Answer the questions on this problem sheet. Write up a short report on your findings, with answers to the questions. Make sure to include relevant graphs produced and the R code you used.*


Remember that this is assessed work, and your report must be submitted by Thursday 26 March. You might like to look at [my example solution to Computational Project 1 (HTML)](https://mpaldridge.github.io/math2750/C1-example.html) ([Rmd file](https://mpaldridge.github.io/math2750/C1-example.Rmd)) to see what a good report looks like.



***



We start by reading three functions into R by entering the entire text of the following code chunk. The easiest way to do this is by using the [`C2.Rmd` file](https://mpaldridge.github.io/math2750/C2.Rmd) in RStudio, and pressing the green "play" button next to the following code chunk. Alternatively, copying-and-pasting from `C2.html` will probably work.

```{r}
# Produces a transition matrix for a Markov chain based on a parameter eps
TransMat <- function(eps) {
  matrix(c(0.1, 0.9,   0,       0,       0,   0,
           0.4, 0.2, 0.4,       0,       0,   0,
             0, 0.5,   0,     0.5,       0,   0,
             0,   0, 0.5, 0.5-eps,     eps,   0,
             0,   0,   0,     eps, 0.3-eps, 0.7,
             0,   0,   0,       0,     0.5, 0.5),
         nrow = 6, ncol = 6, byrow = TRUE)
}


# Simulates a Markov chain with transition matrix P for N steps from X0
MarkovChain <- function(N, X0, P) {
  X <- numeric(N)   # empty vector of length N
  space <- nrow(P)  # number of points in sample space
  
  now <- X0
  for (n in 1:N) {
    now <- sample(space, 1, prob = P[now, ])
    X[n] <- now
  }
  
  X
}


# Finds the stationary distribution of a Markov chain
StatDist <- function(P) {
  LeftEigen <- eigen(t(P))
  statmeas <- LeftEigen$vectors[, 1]  # Unnormalised stationary measure
  statmeas / sum(statmeas)            # Normalised stationary distribution
}
```

It is not important that you understand how these functions work, but you do need to know what they do.

The first function **`TransMat`** produces a transition matrix
\[ \begin{pmatrix} 
     0.1 & 0.9 &   0 &            0 &            0 &   0 \\
     0.4 & 0.2 & 0.4 &            0 &            0 &   0 \\
       0 & 0.5 &   0 &          0.5 &            0 &   0 \\
       0 &   0 & 0.5 & 0.5-\epsilon &     \epsilon &   0 \\
       0 &   0 &   0 &     \epsilon & 0.3-\epsilon & 0.7 \\
       0 &   0 &   0 &            0 &          0.5 & 0.5
   \end{pmatrix} \]
for some value of the parameter $\epsilon$.

The second function, **`MarkovChain`**, simulates a Markov chain. For example, `MarkovChain(10, 2, P)` simulates a Markov chain with transition matrix `P`, starting from state 2 for 10 steps.

The third function, **`StatDist`**, calculates a stationary distribution for a given transition matrix (see Lecture 10 of the course -- we may not have covered this yet). This function works for any *irreducible* Markov chain on a finite state space -- it may behave strangely for nonirreducible Markov chains.



***



We will be investigating the Markov chain with $\epsilon = 0.2$.



**Question 1.** *Using the function `TransMat`, produce the transition matrix for $\epsilon = 0.2$. Using the function `MarkovChain`, simulate $N = 50$ steps of this Markov chain starting from $X_0 = 1$. Check it has worked, perhaps by plotting a graph or examining the vector produced. Comment on what you find.*


You will presumably want to try something like this:

<!-- Delete `eval = FALSE` from the next line if you want to run the R code in a R Markdown report. -->
```{r, eval = FALSE}
eps <- 0.2
P   <- TransMat(eps)
N   <- 50
X0  <- 1
X   <- MarkovChain(N, X0, P)
plot(X)
```

If you plot a graph of the Markov chain, you may want to add extra options to label axes, add lines to the points, include the point $X_0 = 1$ etc, as in the previous practical. See the help file `?plot` to remind yourself.



**Question 2.** *Calculate the proportion of time spent in each state. What if you choose $N$ to be very large? How does this compare with the stationary distribution? Relate what you have found to a result in the course.*


One way to count the number of visits to each state is to use the `table` function, although you may be able to think of a different way. The command `table(X)` gives the number of entries in a vetor `X` that have each value. (If your Markov chain did not visit every state, `table(factor(X, 1:6))` will ensure it counts 0 for the unvisited states.)

```{r, eval = FALSE}
num_visits <- table(factor(X, 1:6))
proportion_time <- # your code here
```

Remember that you want the *proportion* of time spent in each state, not just the *number* of visits.

It may also be helpful to illustrate your example with a graph. You will want to give your plot a title, label the axes, and so on. 

You can compare this with the stationary distribution produced by `StatDist`. (See lecture 10 for more on stationary distributions.)

```{r, eval = FALSE}
stationary <- StatDist(P)
rbind(proportion_time, stationary)
```

One way to compare them might be to add points to your plot that represent the stationary distribution with something like this:

```{r, eval = FALSE}
barplot(rbind(proportion_time, stationary), beside = TRUE, legend.text = TRUE)
```

You might want to label your plot or change the legend text -- reading the help file `?barplot` may be helpful here.

A result from the course called *the ergodic theorem* (see Lecture 11) says that when the number of steps $N$ of an irreducible finite-state Markov chain is very large, then the proportion of time spent in each state tends to the stationary distribution. Make sure you have checked what happens with your code for a large value of $N$ -- something like $N = 100\,000$ or even more might be appropriate.

Remember that if you use a certain routine many times, it can be helpful to write it into a function.



**Question 3.** *For each state, estimate the expected return time $\mu_i$. What do you expect the answer to be,? How does your result compare with this?*


(You should remember the expected return time $\mu_i$ from Lecture 8.)

A useful function here might be the `which` function, where `which(v == x)` gives a list of all the coordinates $i$ at which a vector $\mathbf{v}$ took on a particular value $v_i = x$. So, for example, `which(X == 3)` will give a list of all the times when the Markov chain was in state $3$.

Another useful function is the function `diff`, which produces the consecutive differences of a vector; so, for example,
\[ \mathtt{diff}(a, b, c, d, e) = (b - a, c - b, d - c, e - d). \]

Can you use `which` and `diff` together to find the return times to your chosen state, and thus calculate the mean return time?

Another result from the course (see Lecture 10) says that, for an irreducible finite-state Markov chain, the stationary distribution $\boldsymbol\pi$ and the expected return times $\mu_i$ are related by $\pi_i = 1/\mu_i$.



**Question 4.** *What do you think would happen if you were to repeat the above exercises with $\epsilon = 0.01$? What about $\epsilon = 10^{-6}$? What about $\epsilon = 0$? Explain your answers.*


A few sentences here is sufficient (and maybe a diagram); no R coding is required.