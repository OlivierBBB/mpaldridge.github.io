...
We used two functions. First, the "random plus-or-minus" function `rpm(N, p)` gives $N$ random $\pm 1$s.
```{r}
rpm <- function(N, p) {
  Z <- rbinom(N, 1, p)
  2 * Z - 1
...
```
Second, we have the function for the random walk itself
```{r}
...
GamblersRuin <- function(p, a, m) {
  ...
  while (Money > 0 && Money < m) {  # until the game finishes...
    Money <- Money + rpm(1, p)      # ...play another round
  }
...
This example report was written using **R Markdown**, which allows the easy integration of text, LaTeX, displayed R code, and executed R code in one document. You do *not* have to write your report using R Markdown, although some students might like to try. 
...
* The offical [R Markdown guide](https://rmarkdown.rstudio.com/lesson-1.html)

You can download the R markdown source code for this report at <https://mpaldridge.github.io/math2750/C1-sols.Rmd>. If you open that file in RStudio and click on `knit`, it should produce the HTML file as seen at <https://mpaldridge.github.io/math2750/C1-sols.html> --  although note that the R code (including generation of random numbers) is run anew, so your graphs and sample estimates will not be exactly the same.
